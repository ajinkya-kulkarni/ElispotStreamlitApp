...
def min_max_normalize_intensities(intensities):
    if intensities.size == 0:
        return np.array([])  # Return an empty array if intensities is empty
...
if points.size != 0:
    ...
    # Create the figure with the specified size and DPI without scatter plot
    fig = plt.figure(figsize=(4, 4), dpi=DPI)

    # Plot the image without scatter plot
    plt.imshow(display_image, cmap="gray")
...
if points.size != 0:
    # Second subplot: Original image with scatter points colored by intensity
    ...
    # Third subplot: Normalized histogram of spot intensities
    ...
    # If no points, hide the second and third subplots
    axs[1].axis("off")
    axs[1].set_title('No spots detected')
    
    axs[2].axis("off")
    axs[2].set_title('No data')
